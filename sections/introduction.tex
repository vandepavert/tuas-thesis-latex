...
\begin{document}
    ...
    \paragraph{}
    The probability that codeword ${r}$ is received if codeword ${c}$ is transmitted can be expressed as ${P(r|c)}$. In \Cref{fig:binary_symmetric_channel}, a model of a binary symmetric channel (BSC) shows the event that a 1 is transmitted. There is a probability that 0 is received. The transmission is unaltered with a probability of ${1-p}$ \autocite{bossert1999channel}.
